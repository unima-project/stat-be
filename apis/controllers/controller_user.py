...
from utils.util_security import Generate_random_password
...
def Update_user():
    ...
    try:
        ...
        err, current_user = Find_user_by_id(request.get_json()['id'])
        if err:
            logging.error(err)
            error_response['message'] = err
            return jsonify(error_response), 400
...
def Delete_user():
    ...
    try:
        err, current_user = Find_user_by_id(request.args.get('id'))
        if err:
            logging.error(err)
            error_response['message'] = err
            return jsonify(error_response), 400
